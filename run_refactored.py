...
使用开源成熟的组件：
- RTPProxy: 媒体中继（已集成）
- 保留现有SIP信令处理（后续可迁移到Sippy B2BUA）

架构说明：
1. 媒体中继：使用RTPProxy（成熟稳定）
2. SIP信令：当前保留自定义实现，但结构清晰便于迁移
3. 功能保留：CDR、用户管理、MML等全部保留
"""
...
# ====== RTPProxy配置 ======
# RTPProxy TCP地址（从环境变量读取，默认127.0.0.1:7722）
...
# ====== 媒体中继模式 ======
# 使用RTPProxy进行媒体中继（替代自定义RTP转发）
ENABLE_MEDIA_RELAY = True  # 启用媒体中继
...
def _make_response(req: SIPMessage, code: int, reason: str, 
    ...
    r.add_header("server", "ims-sip-server/refactored")
...
def _forward_request(msg: SIPMessage, addr, transport):
    ...
    if method in ("INVITE", "MESSAGE") and _is_initial_request(msg):
        msg.headers.pop("route", None)  # 删除UA自带的Route
